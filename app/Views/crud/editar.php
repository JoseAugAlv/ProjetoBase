...
<section class="crud-section animate-in">
    <div class="container">
        <div class="crud-content">
            ...
            <div class="crud-hero">
                <div>
                    <h1><i class="fas fa-edit" style="color: var(--color-impact-green);"></i> Editar <?= $labelSingular ?></h1>
                    <p class="crud-subtitle">Atualize os dados do <?= strtolower($labelSingular) ?></p>
                </div>
                <span class="badge-count">
                    <i class="fas fa-hashtag"></i> ID #<?= $registro[$this->primaryKey] ?? '' ?>
                </span>
            </div>

            <!-- Formulário -->
            <form action="<?= $basePath ?>/<?= $routePrefix ?>/atualizar" method="POST" class="crud-form">
                <?= ViewHelper::csrfField() ?>
                <input type="hidden" name="<?= $this->primaryKey ?? 'id' ?>" value="<?= $registro[$this->primaryKey ?? 'id'] ?? '' ?>">
